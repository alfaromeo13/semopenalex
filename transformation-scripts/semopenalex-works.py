...
def get_file_folders(s3_client, bucket_name, prefix=""):
    ...
    print("DEBUG: Entering get_file_folders()", flush=True)
    ...
    while next_token is not None:
        ...
        print(f"DEBUG: Calling list_objects_v2 with {updated_kwargs}", flush=True)
        
        response = s3_client.list_objects_v2(**updated_kwargs)

        print("DEBUG: S3 response received.", flush=True)
        
        contents = response.get("Contents")


        print(f"DEBUG: Received {len(contents)} items.", flush=True)
       
        for result in contents:
            key = result.get("Key")
            print(f"DEBUG: Key found: {key}", flush=True)
            if key[-1] == "/":
                folders.append(key)
            else:
        ...
        print("DEBUG: NextContinuationToken =", next_token, flush=True)


    print(f"DEBUG: Total files={len(file_names)} folders={len(folders)}", flush=True)
    return file_names, folders


def download_files(s3_client, bucket_name, local_path, file_names, folders):

    print(f"DEBUG: Starting download of {len(file_names)} files and {len(folders)} folders", flush=True)
    local_path = Path(local_path)

    for folder in folders:
        folder_path = Path.joinpath(local_path, folder)
        print(f"DEBUG: Ensure folder exists {folder_path}", flush=True)
        folder_path.mkdir(parents=True, exist_ok=True)

    for file_name in file_names:

        print(f"DEBUG: Downloading S3:{file_name}", flush=True)

        file_path = Path.joinpath(local_path, file_name)
        file_path.parent.mkdir(parents=True, exist_ok=True)

        print(f"DEBUG: Saving to {file_path}", flush=True)
...
trig_output_dir_path = "/data/graphdb-import/works"

os.makedirs(trig_output_dir_path, exist_ok=True)
...
def transform_gz_file(gz_file_path):
    print(f"[PID {os.getpid()}] Worker started on {gz_file_path}", flush=True)
    print(f"Worker started: {gz_file_path}", flush=True)
    ...
    with open(f"{trig_output_dir_path}/{gz_file_name}.trig", "w", encoding="utf-8") as g:
        with gzip.open(gz_file_path, 'r') as f:
            ...
            for line in f:
                ...
                if i % 50000 == 0:
                    print(f'Processed works entity {i} lines in file {gz_file_path}', flush=True)
